...
sys.path.insert(0, '/users/xpb20111/programs/benchmark_models/models/')
sys.path.insert(0, os.getcwd())

#import score_regression_model as sc
from feat_prep_funcs import rm_const_desc, scale_x, rm_corr_var, rfe_select
from calc_desc import calc_desc_df

# Import all models in models directory, need to give these explicitly
#from models import RF, Ridge
...
if '-type' in sys.argv:
    di['model_type'] = sys.argv[((sys.argv).index('-type'))+1]
    model_type = sys.argv[((sys.argv).index('-type'))+1]
if model_type == 'regression':
    import score_regression_model as sc
elif model_type == 'classification':
    import score_classification_model as sc
else:
    sys.exit('ERROR: -type must be "regression" or "classification"')
...
if '-notes' in sys.argv:
    for i in sys.argv[((sys.argv).index('-notes'))+1:]:
        if i[0] == '-':
            break
        dataset_notes += i
...
if calc_descs:
    ...
    df_desc = calc_desc_df(df[smi_col], 
                           tauto=tauto, 
                           ph=ph, 
                           phmodel=phmodel)
...
def eval_model(trained_model, y_true, y_pred, model_scores, c=None, cls_labels=None):
...
def train_eval_model(ml_model, x, y, hyper_cv, n_jobs=-1, save_model=False):

    trained_model, cv_tuning_stats[ml_model][n], tuned_params[ml_model][n] = train_model(ml_model, x['train'], y['train'], hyper_cv, n_jobs)
    if save_model:
	    pk.dump(trained_models, open(ml_model+'_refit.pk', 'wb'))

    for data_split in ['test', 'train']:
        y_pred = trained_model.predict(x[data_split]).squeeze()
        model_scores[ml_model][data_split] = eval_model(trained_model, y[data_split], y_pred, model_scores[ml_model][data_split], c=c_split[data_split], cls_labels=cls_labels)
...
if '-refit' in sys.argv:
    ...
    x, n_const_descs = rm_const_desc(x)
    ...
    if '-ext_test' in sys.argv:
       ...
       for ext_name, data_file in ext_ls:
           ...
           if calc_descs:
               ...
               df_desc = calc_desc_df(df[smi_col], tauto, ph, desc_ls=x.columns.to_list(), rm_na=False)
    ...
    for ml_model in np.array(ml_model_order)[~rfe_mask]:
        ...
        model_scores[ml_model] = {'train' : {}}
        for metric in metric_ls:
            model_scores[ml_model]['train'][metric] = []

        trained_models[ml_model], cv_tuning_stats[ml_model], tuned_params[ml_model] = train_model(ml_model, x, y, hyper_cv, n_jobs)
        y_pred = trained_models[ml_model].predict(x)
        model_scores[ml_model]['train'] = sc.calc_model_scores(y, y_pred, model_scores[ml_model]['train'])
        ...
        if '-ext_test' in sys.argv:
            for ext_name in ext_ls[:,0]:
                ...
                pred_out = open(ml_model+'_'+ext_name+'_preds.dat', 'w')
                pred_out.write(dlim.join([str(i) for i in y_ext[ext_name].index]))
                pred_out.write(dlim.join([str(i) for i in y_pred]))
                pred_out.close()
                model_scores[ml_model][ext_name] = {}
                for metric in metric_ls:
                    model_scores[ml_model][ext_name][metric] = []
                model_scores[ml_model] = sc.calc_model_scores(y_ext[ext_name], y_pred, model_scores[ml_model][ext_name])
        ...
        for ml_model in np.array(ml_model_order)[rfe_mask]:
            ...
            if '-ext_test' in sys.argv:
                for ext_name in ext_ls[:,0]:
                    ...
                    for metric in metric_ls:
                        model_scores[ml_model][ext_name][metric] = []
                    model_scores[ml_model][ext_name] = sc.calc_model_scores(y_ext[ext_name], y_pred, model_scores[ml_model][ext_name])

    for ml_model in ml_model_order:
        # Write hyper parameters and predictions from each MC CV cycle:
        out = open(ml_model+'_refit_ext_stats.dat', 'w')

        out.write('Dataset'+dlim)
        ...
        out.write('Refit_model'+dlim)

        # No test set since all data used for retraining:
        out.write(dlim.join([str(np.nan) for metric in metric_ls]))

        out.write("{}{}".format(dlim, n_const_descs))
        out.write("{}{}".format(dlim, n_corr_var))
        if True in rfe_mask:
            out.write("{}{}{}{}".format(dlim, x.shape[1],
                                        dlim, x.columns))

        for metric in metric_ls:
            out.write("{}{:.5e}".format(dlim, model_scores[ml_model]['train'][metric][0]))

        out.write("{}{}".format(dlim, tuned_params[ml_model][0]))
        out.write("{}{}".format(dlim, cv_tuning_stats[ml_model][0]))
        out.write("\n")
    ...
    if '-ext_test' in sys.argv:
        for ml_model in ml_model_order:
            ...
            for ext_name, data_file in ext_ls:
                ...
                out.write("{}".format(ext_name))

                # Performance metrics:
                for metric in metric_ls:
                    out.write("{}{:.5e}".format(dlim, model_scores[ml_model][ext_name][metric][0]))

                # Probably better to put these in a separate file:
                # Final features (from RFE and removing correlated variables):
                out.write("{}{}".format(dlim, n_rm_desc))
                out.write("{}{}".format(dlim, n_corr_var))
                ...
                for metric in metric_ls:
                    out.write("{}{:.5e}".format(dlim, model_scores[ml_model][metric]))
